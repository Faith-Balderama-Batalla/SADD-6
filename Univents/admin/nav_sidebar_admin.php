...
// Admin sidebar navigation - Make sure $user and $unread_count are defined
?>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div class="sidebar-menu">
            ...
            <a href="pending.php" class="menu-item <?php echo basename($_SERVER['PHP_SELF']) == 'pending.php' ? 'active' : ''; ?>" data-tooltip="Pending Approvals">
                ...
                $pending_count = 0;
                if (isset($conn)) {
                    $count_stmt = $conn->prepare("SELECT COUNT(*) as c FROM pending_officers WHERE status = 'pending'");
                    $count_stmt->execute();
                    $pending_count = $count_stmt->get_result()->fetch_assoc()['c'] ?? 0;
                    $count_stmt->close();
                }
                if($pending_count > 0): ?>
